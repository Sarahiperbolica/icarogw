        ...
        with h5py.File(outfile,'a') as icat:
            ...
            for pix in tqdm(filled_pixels,desc='Finding a common redshift grid among pixels'):
                with h5py.File(os.path.join(outfolder,'pixel_{:d}.hdf5'.format(pix)),'r') as subcat:
                    ...
                    # Se il dataset non esiste, questo pixel NON contribuisce
                    #if 'valid_galaxies_interpolant' not in grp:
...
class icarogw_catalog(object):
    ...
    def build_from_pixelated_files(self, outfolder):
        ...
        if len(filled_pixels) == 0:
            raise RuntimeError(
                "No filled pixels found in catalog. "
                "Check that calc_interpolant step produced output."
            )
    
        # --- Initialize Schechter function once, before looping ---
        with h5py.File(os.path.join(outfolder, f"pixel_{filled_pixels[0]}.hdf5")) as pcat:
        ...
        # --- Prepare sky grid ---
        self.sky_grid = np.arange(len(self.moc_mthr_map.data), dtype=int)
        dNgal_dzdOm_vals = []
        bg_vals = []
    
        # --- Loop over pixels ---
        for pix in tqdm(self.sky_grid, desc='Building sky grid'):
