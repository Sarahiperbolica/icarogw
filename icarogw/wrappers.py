...
lcdm_wrappers = ["FlatLambdaCDM_wrap", "FlatwCDM_wrap", "Flatw0waCDM_wrap"]
...
class eps0_mod_wrap(object):
    ...
    def update(self, **kwargs):
        ...
        self.cosmology.build_cosmology(
            self.bgwrap.astropycosmo(**bgdict), eps0=kwargs["eps0"]
        )
...
class mass_ratio_prior_Gaussian(pm_prob):
    ...
    def update(self, **kwargs):
        p1 = TruncatedGaussian(kwargs["mu_q"], kwargs["sigma_q"], 0.0, 1.0)
        self.prior = p1
...
class mass_ratio_prior_Powerlaw(pm_prob):
    ...
    def update(self, **kwargs):
        self.prior = PowerLaw(0.0, 1.0, kwargs["alpha_q"])
...
class massprior_MultiPeak(pm_prob):
    ...
    def update(self, **kwargs):
        self.prior = PowerLawTwoGaussians(
            kwargs["mmin"],
            kwargs["mmax"],
            -kwargs["alpha"],
            kwargs["lambda_g"],
...
class massprior_BrokenPowerLawMultiPeak(pm_prob):
    ...
    def update(self, **kwargs):
        self.prior = BrokenPowerLawMultiPeak(
            kwargs["mmin"],
            kwargs["mmax"],
            -kwargs["alpha_1"],
            -kwargs["alpha_2"],
...
class massprior_BrokenPowerLawTripleMultiPeak(pm_prob):
    ...
    def update(self, **kwargs):
        self.prior = BrokenPowerLawTripleMultiPeak(
            kwargs["mmin"],
            kwargs["mmax"],
            -kwargs["alpha_1"],
            -kwargs["alpha_2"],
            ...
            kwargs["sigma_g_1"],
            kwargs["mmin"],
            kwargs["mu_g_1"] + 5 * kwargs["sigma_g_1"],
            kwargs["mu_g_2"],
            kwargs["sigma_g_2"],
            kwargs["mmin"],
            kwargs["mu_g_2"] + 5 * kwargs["sigma_g_2"],
            kwargs["mu_g_3"],
            kwargs["sigma_g_3"],
            kwargs["mmin"],
            kwargs["mu_g_3"] + 5 * kwargs["sigma_g_3"],
        )
...
class m1m2_conditioned(pm1m2_prob):
    ...
    def update(self, **kwargs):
        self.wrapper_m.update(
            **{key: kwargs[key] for key in self.wrapper_m.population_parameters}
        )
...
class m1m2_paired_massratio_dip(pm1m2_prob):
    ...
    def update(self, **kwargs):
        ...
        p = SmoothedPlusDipProb(
            self.wrapper_m.prior,
            **{
                key: kwargs[key]
                for key in [
                    "bottomsmooth",
...
class m1m2_paired_massratio_dip_general(pm1m2_prob):
    ...
    def update(self, **kwargs):
        ...
        def pairing_function(
            m1,
            m2,
            beta_bottom=kwargs["beta_bottom"],
            beta_top=kwargs["beta_top"],
...
class m1m2_paired_massratio_bpl_dip_farah_2022(pm1m2_prob):
    ...
    def update(self, **kwargs):
        kwargs["b"] = (kwargs["leftdip"] - kwargs["mmin"]) / (
            kwargs["mmax"] - kwargs["mmin"]
        )
        self.wrapper_m.update(
            **{key: kwargs[key] for key in self.wrapper_m.population_parameters + ["b"]}
        )
        p = SmoothedPlusDipProb(
            self.wrapper_m.prior,
            **{
        ...
        def pairing_function(
            m1, m2, beta_bottom=kwargs["beta_bottom"], beta_top=kwargs["beta_top"]
        ):
...
class massprior_BinModel2d(pm1m2_prob):
    ...
    def update(self, **kwargs):
        kwargs_bin_parameters = np.array(
...
class spinprior_default_evolving_gaussian(object):
    ...
    def log_pdf(self, chi_1, chi_2, cos_t_1, cos_t_2, mass_1_source, mass_2_source):
        ...
        a, b = (0.0 - mu_chi_1) / sigma_chi_1, (1.0 - mu_chi_1) / sigma_chi_1
        g1 = sx.stats.truncnorm.pdf(chi_1, a, b, loc=mu_chi_1, scale=sigma_chi_1)

        # compute the truncated gaussian referred to chi2
        a, b = (0.0 - mu_chi_2) / sigma_chi_2, (1.0 - mu_chi_2) / sigma_chi_2
        g2 = sx.stats.truncnorm.pdf(chi_2, a, b, loc=mu_chi_2, scale=sigma_chi_2)

        # implement eq. 12 of https://arxiv.org/pdf/2406.01679
        log_angular_part = xp.logaddexp(
            xp.log1p(-self.csi_spin) + xp.log(0.25),
            xp.log(self.csi_spin)
            + self.aligned_pdf.log_pdf(cos_t_1)
            + self.aligned_pdf.log_pdf(cos_t_2),
        )
    ...
    def pdf(self, chi_1, chi_2, cos_t_1, cos_t_2, mass_1_source, mass_2_source):
        xp = get_module_array(chi_1)
        return xp.exp(
            self.log_pdf(chi_1, chi_2, cos_t_1, cos_t_2, mass_1_source, mass_2_source)
        )
...
class spinprior_default_beta_window_gaussian(object):
    ...
    def update(self, **kwargs):
        ...
        self.sigma_chi = kwargs["sigma_chi"]
        self.csi_spin = kwargs["csi_spin"]
        self.gaussian_pdf_chi = TruncatedGaussian(
            kwargs["mu_chi"], kwargs["sigma_chi"], 0.0, 1.0
        )

        self.mt, self.delta_mt, self.mix_f = (
            kwargs["mt"],
            kwargs["delta_mt"],
            kwargs["mix_f"],
        )
    ...
    def log_pdf(self, chi_1, chi_2, cos_t_1, cos_t_2, mass_1_source, mass_2_source):
        ...
        wz_1 = _mixed_double_sigmoid_function(
            mass_1_source, self.mix_f, 0.0, self.mt, self.delta_mt
        )
        wz_2 = _mixed_double_sigmoid_function(
            mass_2_source, self.mix_f, 0.0, self.mt, self.delta_mt
        )

        # compute p(chi_i|m_i,Lambda)
        pdf_1 = wz_1 * self.beta_pdf_chi.pdf(chi_1) + (
            1 - wz_1
        ) * self.gaussian_pdf_chi.pdf(chi_1)
        pdf_2 = wz_2 * self.beta_pdf_chi.pdf(chi_2) + (
            1 - wz_2
        ) * self.gaussian_pdf_chi.pdf(chi_2)
...
class spinprior_default_beta_window_beta(object):
    ...
    def update(self, **kwargs):
        ...
        if (
            (self.alpha_chi_low <= 1)
            | (self.beta_chi_low <= 1)
            | (self.alpha_chi_high <= 1)
            | (self.beta_chi_high <= 1)
        ):
            raise ValueError("Alpha and Beta must be > 1")

        self.beta_pdf_chi_low = BetaDistribution(self.alpha_chi_low, self.beta_chi_low)
        self.beta_pdf_chi_high = BetaDistribution(
            self.alpha_chi_high, self.beta_chi_high
        )
    ...
    def log_pdf(self, chi_1, chi_2, cos_t_1, cos_t_2, mass_1_source, mass_2_source):
        ...
        pdf_1 = wz_1 * self.beta_pdf_chi_low.pdf(chi_1) + (
            1 - wz_1
        ) * self.beta_pdf_chi_high.pdf(chi_1)
        pdf_2 = wz_2 * self.beta_pdf_chi_low.pdf(chi_2) + (
            1 - wz_2
        ) * self.beta_pdf_chi_high.pdf(chi_2)
...
class spinprior_default_gaussian(object):
    ...
    def update(self, **kwargs):
        self.csi_spin = kwargs["csi_spin"]
        self.aligned_pdf = TruncatedGaussian(1.0, kwargs["sigma_t"], -1.0, 1.0)
        self.g1 = TruncatedGaussian(kwargs["mu_chi_1"], kwargs["sigma_chi_1"], 0.0, 1.0)
        self.g2 = TruncatedGaussian(kwargs["mu_chi_2"], kwargs["sigma_chi_2"], 0.0, 1.0)
...
class pseobprior_gaussian(object):
    ...
    def update(self, **kwargs):
        ...
        self.pdf_evaluator = Bivariate2DGaussian(
            x1min=-10,
            x1max=10.0,
            x1mean=kwargs["mu_domega220"],
            x2min=-10,
            x2max=10.0,
            x2mean=kwargs["mu_dtau220"],
            x1variance=kwargs["sigma_domega220"] ** 2.0,
            x12covariance=kwargs["rho_pseob"]
            * kwargs["sigma_domega220"]
            * kwargs["sigma_dtau220"],
            x2variance=kwargs["sigma_dtau220"] ** 2.0,
        )
...
class PowerLaw_PowerLaw_PowerLaw:
    ...
    def pdf(self, m):
        ...
        if (
            ...
            or (self.mix_beta > 1)
            or (self.mix_beta < 0)
            or (self.mix_alpha + self.mix_beta > 1)
        ):
            return xp.nan
        else:
            return (
                ...
                + (1 - self.mix_beta - self.mix_alpha) * powerlaw_part_c
            )
    ...
    def pdf(self, m):
        ...
        if (
            ...
            return (
                self.mix_alpha * powerlaw_part_a
                + self.mix_beta * powerlaw_part_b
                + (1 - self.mix_beta - self.mix_alpha) * gaussian_part
            )
...
class PowerLaw_GaussianRedshiftLinear:
    ...
    def __init__(
        self,
        redshift_transition="linear",
        flag_powerlaw_smoothing=1,
        flag_redshift_mixture=1,
    ):
        ...
        if self.flag_redshift_mixture:
            ...
            if self.redshift_transition == "sigmoid":
                self.population_parameters += ["zt", "delta_zt"]
        if self.flag_powerlaw_smoothing:
            self.population_parameters += ["delta_m"]
    ...
    def update(self, **kwargs):
        ...
        if self.flag_redshift_mixture:
            ...
            if self.redshift_transition == "sigmoid":
                self.zt, self.delta_zt = kwargs["zt"], kwargs["delta_zt"]
        if self.flag_powerlaw_smoothing:
    ...
    def pdf(self, m, z):
        ...
        if self.flag_redshift_mixture:
            if self.redshift_transition == "linear":
                ...
                wz = _mixed_double_sigmoid_function(
                    z, self.mix_z0, self.mix_z1, self.zt, self.delta_zt
                )
            else:
                raise ValueError(
                    "The slected redshift transition model {} does not exist. Exiting.".format(
                        self.redshift_transition
                    )
                )
        else:
            wz = self.mix_z0
        ...
        gaussian_class = GaussianLinear(
            z, self.mu_z0, self.mu_z1, self.sigma_z0, self.sigma_z1, self.mmin
        )
...
class PowerLaw_GaussianRedshiftLinear_GaussianRedshiftLinear:
    ...
    def pdf(self, m, z):
        ...
        if self.flag_redshift_mixture:
            if self.redshift_transition == "linear":
                wz_alpha = _mixed_linear_function(
                    z, self.mix_alpha_z0, self.mix_alpha_z1
                )
                wz_beta = _mixed_linear_function(z, self.mix_beta_z0, self.mix_beta_z1)
            elif self.redshift_transition == "sigmoid":
                wz_alpha = _mixed_double_sigmoid_function(
                    z, self.mix_alpha_z0, self.mix_alpha_z1, self.zt, self.delta_zt
                )
                wz_beta = _mixed_double_sigmoid_function(
                    z, self.mix_beta_z0, self.mix_beta_z1, self.zt, self.delta_zt
                )
            else:
                raise ValueError(
                    "The slected redshift transition model {} does not exist. Exiting.".format(
        ...
        if self.flag_powerlaw_smoothing:
            powerlaw_class = LowpassSmoothedProb(powerlaw_class, self.delta_m)
        gaussian_a_class = GaussianLinear(
            z, self.mu_a_z0, self.mu_a_z1, self.sigma_a_z0, self.sigma_a_z1, self.mmin
        )
        gaussian_b_class = GaussianLinear(
            z, self.mu_b_z0, self.mu_b_z1, self.sigma_b_z0, self.sigma_b_z1, self.mmin
        )
        powerlaw_part = powerlaw_class.pdf(m)
        ...
        if (
            (xp.any(wz_alpha > 1))
            or (xp.any(wz_alpha < 0))
            or (xp.any(wz_beta > 1))
            or (xp.any(wz_beta < 0))
            or (xp.any(wz_alpha + wz_beta > 1))
        ):
            return xp.nan
        else:
            return (
                wz_alpha * powerlaw_part
                + wz_beta * gaussian_a_part
                + (1 - wz_beta - wz_alpha) * gaussian_b_part
            )
...
class PowerLawRedshiftLinear_GaussianRedshiftLinear:
    ...
    def pdf(self, m, z):
        ...
        gaussian_class = GaussianLinear(
            z, self.mu_z0, self.mu_z1, self.sigma_z0, self.sigma_z1, self.mmin_z0
        )
        powerlaw_part = powerlaw_class.pdf(m)
        gaussian_part = gaussian_class.pdf(m)
...
class PowerLawRedshiftLinear_PowerLawRedshiftLinear_PowerLawRedshiftLinear:
    ...
    def pdf(self, m, z):
        ...
        powerlaw_class_a = PowerLawLinear(
            z,
            self.alpha_a_z0,
            self.alpha_a_z1,
            self.mmin_a_z0,
        ...
        powerlaw_class_b = PowerLawLinear(
            z,
            self.alpha_b_z0,
            self.alpha_b_z1,
            self.mmin_b_z0,
        ...
        powerlaw_class_c = PowerLawLinear(
            z,
            self.alpha_c_z0,
            self.alpha_c_z1,
            self.mmin_c_z0,
        ...
        # distribution p(m1|z) needs to be computed at all redshifts corresponding to the PE samples and injections.
        if self.flag_powerlaw_smoothing:
            powerlaw_class_a = LowpassSmoothedProbEvolving(
                powerlaw_class_a, self.delta_m_a
            )
            powerlaw_class_b = LowpassSmoothedProbEvolving(
                powerlaw_class_b, self.delta_m_b
            )
            powerlaw_class_c = LowpassSmoothedProbEvolving(
                powerlaw_class_c, self.delta_m_c
            )
        powerlaw_part_a = powerlaw_class_a.pdf(m)
        powerlaw_part_b = powerlaw_class_b.pdf(m)
        powerlaw_part_c = powerlaw_class_c.pdf(m)
        ...
        if (
            ...
            return (
                wz_alpha * powerlaw_part_a
                + wz_beta * powerlaw_part_b
                + (1 - wz_beta - wz_alpha) * powerlaw_part_c
            )
...
class PowerLawRedshiftLinear_PowerLawRedshiftLinear_GaussianRedshiftLinear:
    ...
    def pdf(self, m, z):
        ...
        gaussian_class = GaussianLinear(
            z, self.mu_z0, self.mu_z1, self.sigma_z0, self.sigma_z1, self.mmin_a_z0
        )
        # Add left smoothing to the evolving PowerLaw.
        # WARNING: The implementation is very slow, because the integral to normalise the windowed
        # distribution p(m1|z) needs to be computed at all redshifts corresponding to the PE samples and injections.
        ...
        if (
            ...
            return (
                wz_alpha * powerlaw_part_a
                + wz_beta * powerlaw_part_b
                + (1 - wz_beta - wz_alpha) * gaussian_part
            )
...
class GaussianRedshiftLinear_GaussianRedshiftLinear:
    ...
    def pdf(self, m, z):
        ...
        gaussian_a_class = GaussianLinear(
            z, self.mu_a_z0, self.mu_a_z1, self.sigma_a_z0, self.sigma_a_z1, self.mmin_g
        )
        gaussian_b_class = GaussianLinear(
            z, self.mu_b_z0, self.mu_b_z1, self.sigma_b_z0, self.sigma_b_z1, self.mmin_g
        )
        gaussian_a_part = gaussian_a_class.pdf(m)
        gaussian_b_part = gaussian_b_class.pdf(m)
...
class GaussianRedshiftLinear_GaussianRedshiftLinear_GaussianRedshiftLinear:
    ...
    def pdf(self, m, z):
        ...
        gaussian_c_class = GaussianLinear(
            z, self.mu_c_z0, self.mu_c_z1, self.sigma_c_z0, self.sigma_c_z1, self.mmin_g
        )
        gaussian_a_part = gaussian_a_class.pdf(m)
        gaussian_b_part = gaussian_b_class.pdf(m)
        ...
        if (
            ...
            return (
                wz_alpha * gaussian_a_part
                + wz_beta * gaussian_b_part
                + (1 - wz_beta - wz_alpha) * gaussian_c_part
            )
...
class GaussianEvolving:
    ...
    def log_pdf(self, m, z):
        ...
        a, b = (0.0 - self.muz) / self.sigmaz, (
            self.muz + 6 * self.muz - self.muz
        ) / self.sigmaz  # Truncte the Gaussian at zero and mu+6*sigma. This improve the numerical stability.
        gaussian = xp.log(
            sx.stats.truncnorm.pdf(m, a, b, loc=self.muz, scale=self.sigmaz)
        )
        return gaussian
...
class spinprior_default_corr_gaussian_window_corr_gaussian(object):
    ...
    def log_pdf(self, chi_eff, chi_p, mass_1_source, mass_2_source):
        ...
        pdf = wz_M * self.gaussian_low.pdf(chi_eff, chi_p) + (
            1 - wz_M
        ) * self.gaussian_high.pdf(chi_eff, chi_p)
...
class spinprior_HBH(object):
    ...
    def log_pdf(self, chi_1, chi_2, cos_t_1, cos_t_2, mass_1_source, mass_2_source):
        ...
        p1g1g = (
            log_tgaussian(chi_1, 0.0, 1.0, 0.0, self.chi_max_1g)
            + log_tgaussian(chi_2, 0.0, 1.0, 0.0, self.chi_max_1g)
            - xp.log(2.0)
            - xp.log(2.0)
        )

        p2g1g = (
            log_tgaussian(chi_1, 0.0, 1.0, self.mean_chi_2g, self.sigma_chi_2g)
            + log_tgaussian(chi_2, 0.0, 1.0, 0.0, self.chi_max_1g)
            - xp.log(2.0)
            - xp.log(2.0)
        )
        ...
        p1g2g = (
            log_tgaussian(chi_2, 0.0, 1.0, self.mean_chi_2g, self.sigma_chi_2g)
            + log_tgaussian(chi_1, 0.0, 1.0, 0.0, self.chi_max_1g)
            - xp.log(2.0)
            - xp.log(2.0)
        )
        ...
        f1gm1 = _mixed_double_sigmoid_function(
            mass_1_source,
            self.f_1g_mt,
            self.f_1g_delta_mt,
            self.f_1g_m_0,
        ...
        f1gm2 = _mixed_double_sigmoid_function(
            mass_2_source,
            self.f_1g_mt,
            self.f_1g_delta_mt,
            self.f_1g_m_0,
        ...
        pout = (
            f1gm1 * f1gm2 * xp.exp(p1g1g)
            + f1gm1 * (1 - f1gm2) * xp.exp(p1g2g)
            + (1 - f1gm1) * f1gm2 * xp.exp(p2g1g)
            + (1 - f1gm1) * (1 - f1gm2) * xp.exp(p2g2g)
        )
    ...
    def sample(self, N, mass_1_source, mass_2_source):
        ...
        n_m1 = len(mass_1_source)
        n_m2 = len(mass_2_source)

        cos_t_1_sample = xp.random.uniform(-1, 1, size=(n_m1, N))
        cos_t_2_sample = xp.random.uniform(-1, 1, size=(n_m2, N))
        ...
        chi_vals = xp.linspace(0, 1, 1000)
        pdf_1g = xp.exp(log_tgaussian(chi_vals, 0, 1, 0, self.chi_max_1g))
        pdf_2g = xp.exp(
            log_tgaussian(chi_vals, 0, 1, self.mean_chi_2g, self.sigma_chi_2g)
        )

        def make_cdf(f):
            pdf = f[:, None] * pdf_1g + (1 - f)[:, None] * pdf_2g  # (n_mass, K)
            cdf = xp.cumsum(pdf, axis=1)
            cdf /= cdf[:, -1][:, None]  # normalize
            return cdf

        cdf1 = make_cdf(f1gm1)
        cdf2 = make_cdf(f1gm2)

        u1 = xp.random.rand(n_m1, N)
        u2 = xp.random.rand(n_m2, N)

        chi_1_samples = xp.array(
            [xp.interp(u1[i], cdf1[i], chi_vals) for i in range(n_m1)]
        )
        chi_2_samples = xp.array(
            [xp.interp(u2[j], cdf2[j], chi_vals) for j in range(n_m2)]
        )

        return chi_1_samples, chi_2_samples, cos_t_1_sample, cos_t_2_sample
...
class spinprior_AGN(object):
    ...
    def log_pdf(self, chi_1, chi_2, cos_t_1, cos_t_2, mass_1_source, mass_2_source):
        ...
        delta_1 = self.delta_0 + self.delta_dot_0 * (mass_1_source / 30.0)
        delta_2 = self.delta_0 + self.delta_dot_0 * (mass_2_source / 30.0)

        # Clip to minimal and maximum values for numerical stability
        delta_1 = np.clip(delta_1, self.mindelta_AGN, 4.0)
        delta_2 = np.clip(delta_2, self.mindelta_AGN, 4.0)

        # Chi distributions
        p1g1g = log_tgaussian(chi_1, 0.0, 1.0, 0.0, self.chi_max_1g) + log_tgaussian(
            chi_2, 0.0, 1.0, 0.0, self.chi_max_1g
        )
        p2g1g = log_tgaussian(
            chi_1, 0.0, 1.0, self.mean_chi_2g, self.sigma_chi_2g
        ) + log_tgaussian(chi_2, 0.0, 1.0, 0.0, self.chi_max_1g)
        p1g2g = log_tgaussian(
            chi_2, 0.0, 1.0, self.mean_chi_2g, self.sigma_chi_2g
        ) + log_tgaussian(chi_1, 0.0, 1.0, 0.0, self.chi_max_1g)
        p2g2g = log_tgaussian(
            chi_2, 0.0, 1.0, self.mean_chi_2g, self.sigma_chi_2g
        ) + log_tgaussian(chi_1, 0.0, 1.0, self.mean_chi_2g, self.sigma_chi_2g)
        ...
        a1 = log_tgaussian(cos_t_1, -1.0, 1.0, 1.0, delta_1)
        a2 = log_tgaussian(cos_t_2, -1.0, 1.0, 1.0, delta_2)
...
class spinprior_PBH_smeared(object):
    ...
    def log_pdf(self, chi_1, chi_2, cos_t_1, cos_t_2, mass_1_source, mass_2_source):
        ...
        out = (
            log_tgaussian(chi_1, 0.0, 1.0, mu_chi_1, self.sigma_chi_PBH)
            + log_tgaussian(chi_2, 0.0, 1.0, mu_chi_2, self.sigma_chi_PBH)
            + xp.log(0.5)
            + xp.log(0.5)
        )
    ...
    def sample(self, N, mass_1_source, mass_2_source):
        ...
        pdf1 = xp.exp(
            log_tgaussian(
                chi_vals[None, :], 0.0, 1.0, mu_chi_1[:, None], self.sigma_chi_PBH
            )
        )
        pdf2 = xp.exp(
            log_tgaussian(
                chi_vals[None, :], 0.0, 1.0, mu_chi_2[:, None], self.sigma_chi_PBH
            )
        )

        cdf1 = xp.cumsum(pdf1, axis=1)
        cdf1 /= cdf1[:, -1][:, None]

        cdf2 = xp.cumsum(pdf2, axis=1)
        cdf2 /= cdf2[:, -1][:, None]
...
class spinprior_IBH(object):
    ...
    def log_pdf(self, chi_1, chi_2, cos_t_1, cos_t_2, mass_1_source, mass_2_source):
        ...
        delta_1 = self.delta_0 + self.delta_dot_0 * (mass_1_source / 30.0)
        delta_2 = self.delta_0 + self.delta_dot_0 * (mass_2_source / 30.0)
        chi_IBH_max_1 = self.chi_IBH_0 + self.chi_dot_IBH_0 * (mass_1_source / 30.0)
        chi_IBH_max_2 = self.chi_IBH_0 + self.chi_dot_IBH_0 * (mass_2_source / 30.0)

        # Clip to minimal and maximum values for numerical stability
        delta_1 = np.clip(delta_1, self.mindelta, 4.0)
        delta_2 = np.clip(delta_2, self.mindelta, 4.0)
        chi_IBH_max_1 = np.clip(chi_IBH_max_1, self.minchi, 2.0)
        chi_IBH_max_2 = np.clip(chi_IBH_max_2, self.minchi, 2.0)

        c1 = log_tgaussian(chi_1, 0.0, 1.0, 0.0, chi_IBH_max_1)
        c2 = log_tgaussian(chi_2, 0.0, 1.0, 0.0, chi_IBH_max_2)
        a1 = log_tgaussian(cos_t_1, -1.0, 1.0, 1.0, delta_1)
        a2 = log_tgaussian(cos_t_2, -1.0, 1.0, 1.0, delta_2)
        out = a1 + a2 + c1 + c2
    ...
    def pdf(self, chi_1, chi_2, cos_t_1, cos_t_2, mass_1_source, mass_2_source):
        ...
        #     xp.exp(
        #         self.log_pdf(
        #             chi_1, chi_2, cos_t_1, cos_t_2, mass_1_source, mass_2_source
        #         )
        #     )
        # )
    ...
    def sample(self, N, mass_1_source, mass_2_source):
        ...
        delta_1 = np.clip(
            self.delta_0 + self.delta_dot_0 * (mass_1_source / 30.0), self.mindelta, 4.0
        )
        delta_2 = np.clip(
            self.delta_0 + self.delta_dot_0 * (mass_2_source / 30.0), self.mindelta, 4.0
        )
        chi_IBH_max_1 = np.clip(
            self.chi_IBH_0 + self.chi_dot_IBH_0 * (mass_1_source / 30.0),
            self.minchi,
            2.0,
        )
        chi_IBH_max_2 = np.clip(
            self.chi_IBH_0 + self.chi_dot_IBH_0 * (mass_2_source / 30.0),
            self.minchi,
            2.0,
        )

        # Grid for chi and cos(theta)
        chi_vals = xp.linspace(0, 1, 1000)
        cos_vals = xp.linspace(-1, 1, 1000)

        # --- Chi sampling ---
        pdf_chi1 = xp.exp(
            log_tgaussian(chi_vals[None, :], 0.0, 1.0, 0.0, chi_IBH_max_1[:, None])
        )
        pdf_chi2 = xp.exp(
            log_tgaussian(chi_vals[None, :], 0.0, 1.0, 0.0, chi_IBH_max_2[:, None])
        )

        cdf_chi1 = xp.cumsum(pdf_chi1, axis=1)
        cdf_chi1 /= cdf_chi1[:, -1][:, None]

        cdf_chi2 = xp.cumsum(pdf_chi2, axis=1)
        cdf_chi2 /= cdf_chi2[:, -1][:, None]

        u1 = xp.random.rand(n_m1, N)
        u2 = xp.random.rand(n_m2, N)

        chi_1_samples = xp.array(
            [xp.interp(u1[i], cdf_chi1[i], chi_vals) for i in range(n_m1)]
        )
        chi_2_samples = xp.array(
            [xp.interp(u2[i], cdf_chi2[i], chi_vals) for i in range(n_m2)]
        )

        # --- cos(theta) sampling ---
        pdf_cos1 = xp.exp(
            log_tgaussian(cos_vals[None, :], -1.0, 1.0, 1.0, delta_1[:, None])
        )
        pdf_cos2 = xp.exp(
            log_tgaussian(cos_vals[None, :], -1.0, 1.0, 1.0, delta_2[:, None])
        )

        cdf_cos1 = xp.cumsum(pdf_cos1, axis=1)
        cdf_cos1 /= cdf_cos1[:, -1][:, None]

        cdf_cos2 = xp.cumsum(pdf_cos2, axis=1)
        cdf_cos2 /= cdf_cos2[:, -1][:, None]

        u_cos1 = xp.random.rand(n_m1, N)
        u_cos2 = xp.random.rand(n_m2, N)

        cos_t_1_samples = xp.array(
            [xp.interp(u_cos1[i], cdf_cos1[i], cos_vals) for i in range(n_m1)]
        )
        cos_t_2_samples = xp.array(
            [xp.interp(u_cos2[i], cdf_cos2[i], cos_vals) for i in range(n_m2)]
        )

        return chi_1_samples, chi_2_samples, cos_t_1_samples, cos_t_2_samples
